...
import { mockEvents } from '@/lib/mock-data';
import { AnimatedElement, AnimatedContainer, AnimatedCard } from '@/components/motion/animated-elements';
  ...
  return (
    <main className="min-h-screen flex flex-col bg-background">
      ...
      <AnimatedElement variant="fadeInUp">
        <section className="py-14 sm:py-16 px-2 sm:px-4 mt-14 md:px-8 bg-card">
         {/* relative sm:mt-14 sm:flex z-10 max-w-6xl mx-auto */}
          ...
          <Link  style={{fontFamily:'Quicksand'}}  href="/contact" className='absolute flex items-center justify-center bg-primary  text-white rounded-lg  lg:hidden top-0 left-5  p-2'>
            Join Us  
          </Link  >
          <span className='  sm:absolute z-10 left-40 top-40'>
            <p  style={{fontFamily:'Quicksand'}} className="text-2xl font-bold sm:text-4xl text-white sm:font-bold mb-4 text-left pl-4 sm:text-center ">
            VOLUNTEERS MAKING A <br /> DIFFERENCE 
          </p>
           
          </span>
          <span className='sm:absolute z-10 right-50 top-20'>
            <p  style={{fontFamily:'Quicksand'}}  className="text-xl lg:hidden text-xlg sm:text-4xl text-white font-bold sm:font-bold mb-4 pl-4 text-left sm:text-center">
           With Over 20+ YEARS OF <br /> IMPACT 
          </p>
            <p  style={{fontFamily:'Quicksand'}} className="text-xl text-xlg sm:text-4xl text-white font-semibold sm:font-bold mb-4 pl-4 text-left sm:text-center">
              With Over  <br />
        ...
        <img src='shape1-4.png' className='hidden sm:inline-block w-[60%] h-[90%] text-green-300 absolute -right-5 -top-5' />
          ...
          <Button  style={{fontFamily:'Quicksand'}} className='absolute  hidden sm:flex bottom-0 sm:-bottom-26 left-[50%]   bg-primary hover:bg-accent/90 text-white px-8 text-lg cursor-pointer font-bold rounded-lg p-5'>
            Join Us  
          </Button>
        ...
        <AnimatedContainer staggerDelay={0.15}>
          <span className='w-full    grid mt-20 sm:flex gap-2 items-center justify-center sm:mt-30 sm:justify-center h-auto sm:h-60 ' >
            {impactMetrics.map((metric, i) => (
              <AnimatedElement key={i} variant="scaleIn" delay={i * 0.1} className='flex flex-col items-center justify-center mx-10'>
      ...
      <AnimatedElement variant="slideInUp">
        ...
        <div
          ...
          <div className='sm:w-2/4 text-white rounded-md sm:rounded-l-md relative p-5 sm:p-20 h-auto sm:h-full bg-green-800'>
            ...
            <p  style={{fontFamily:'Quicksand'}} className='text-orange-300'>Help & Donate</p>
            <h1  style={{fontFamily:'Quicksand'}} className='text-2xl font-bold sm:text-6xl'>Your Small Contribution can Change a Life</h1>
            <p  style={{fontFamily:'Quicksand'}} className='text-sm mx-5 sm:text-base'>Your generous donations help us maintain our work, provide community services, and educate future generations. Every contribution counts and is greatly appreciated.</p>
            ...
            <span className='  mt-10 flex  p-5 w-full border-[0.5px] border-muted-foreground rounded-full'>
              ...
              <p  style={{fontFamily:'Quicksand'}} className='text-sm flex-1 text-right font-semibold text-muted'>
               <b  style={{fontFamily:'Quicksand'}} className='text-orange-400'>$ <AnimatedCounter 
                  value={3546} 
                  duration={2500}
                /></b> raised by <br />
          ...
          <div className='w-auto sm:w-full flex-1 text-white rounded-r-md relative h-auto p-5 sm:p-20 sm:h-full  '>
            <h1  style={{fontFamily:'Quicksand'}} className='text-3xl text-accent font-bold'>Make A Donation</h1>

            <form className='grid gap-5'>
              <span className='w-full grid sm:flex items-center my-5 gap-3 px-3 '>
              <Input  style={{fontFamily:'Quicksand'}} placeholder='Enter your Names' className='sm:flex-1 bg-background text-accent border-0 sm:p-5' />
              <Input  style={{fontFamily:'Quicksand'}} placeholder='Enter your Email Address' className='sm:flex-1 bg-background text-accent border-0 sm:p-7' />
            </span>
            <Input  style={{fontFamily:'Quicksand'}} placeholder='Company name (Optional)' className='sm:flex-1 bg-background text-accent border-0 sm:p-5 sm:my-3 mr-3' />
              <Input  style={{fontFamily:'Quicksand'}} placeholder='Amount Donating' className='sm:flex-1 bg-background text-accent border-0 sm:p-5 my-3 mr-3' />
              <span className='flex w-full justify-evenly items-center gap-3'>
                 {quickdonations.map((donation, i) => (
                  <Button  style={{fontFamily:'Quicksand'}} key={i} variant='outline' className='bg-transparent cursor-pointer border-2 border-green-800 text-green-800 hover:bg-green-800 hover:text-white'>
                    {donation.amount}  
                  </Button>
                ))}
                </span>
      ...
      <AnimatedElement variant="fadeInUp">
        ...
        <h1  style={{fontFamily:'Quicksand'}} className='text-sm     '>Our Events</h1>
        <h2  style={{fontFamily:'Quicksand'}} className='text-3xl sm:text-5xl font-bold text-accent  mb-2'>Be Part of our Upcoming Events</h2>
        <p  style={{fontFamily:'Quicksand'}} className='mb-10 text-muted-foreground'>(Scroll 'UP'  the events to view more!)</p>
    ...
    <ScrollStack className=' relative'>
        {mockEvents.map((event) => (
          <ScrollStackItem  key={event._id}>
           <span className=' text-wrap  items-center justify-evenly grid p-2 sm:w-30 rounded-full h-full bg-primary/10'>
              ...
              <p  style={{fontFamily:'Quicksand'}} className='text-lg text-primary'>{event.date}</p>
            </span>
            
            <span className='flex-1 flex items-center h-full gap-6 px-5'>
              <span className='w-[75%] h-full  '>
                <p  style={{fontFamily:'Quicksand'}} className='text-4xl text-accent font-bold'>{event.title}</p>
                <p  style={{fontFamily:'Quicksand'}} className='text-lg text-muted-foreground'>{event.description}</p>
                <p  style={{fontFamily:'Quicksand'}} className='text-sm  mt-4 font-semibold text-accent'><b className='text-primary'>Topic:</b> {event.topic}</p>
                <p  style={{fontFamily:'Quicksand'}} className='text-sm  font-semibold text-accent'><b className='text-primary'>Time:</b> {event.time}</p>
                <p  style={{fontFamily:'Quicksand'}} className='text-sm  font-semibold text-accent'><b className='text-primary'>Location:</b> {event.location.address}</p>

                <Button  style={{fontFamily:'Quicksand'}} className='bg-primary/20 hover:bg-green-800 hover:text-white text-primary px-8 text-lg cursor-pointer font-bold rounded-lg p-5 mt-5'>
                  Join Now
                </Button>
              </span>
        ...
        <span  className='lg:hidden w-full h-120 bg-card grid gap-2 items-center overflow-y-auto'>
          {mockEvents.map((event) => (
            <div
              ...
              <p  style={{fontFamily:'Quicksand'}} className='text-lg truncate line-clamp-2 mt-4 px-3 text-left font-bold'>{event.title}</p>
              <p  style={{fontFamily:'Quicksand'}} className='text-sm truncate  text-wrap line-clamp-3 text-muted-foreground px-3 text-left  '>{event.description}</p>
              <span  style={{fontFamily:'Quicksand'}} className='text-sm px-3 my-4 font-semibold text-accent'><b className='text-primary'>Topic:</b> {event.topic}</span> <br />
              <span  style={{fontFamily:'Quicksand'}} className='text-sm px-3 my-4 font-semibold text-accent'><b className='text-primary'>Date:</b> {event.date}</span><br />
              <span  style={{fontFamily:'Quicksand'}} className='text-sm px-3 my-4 font-semibold text-accent'><b className='text-primary'>Time:</b> {event.time}</span><br />
              <span  style={{fontFamily:'Quicksand'}} className='text-sm truncate text-wrap line-clamp-2 px-3  mt-2 font-semibold text-accent'><b className='text-primary'>Location:</b> {event.location.address}</span><br />

              <Button  style={{fontFamily:'Quicksand'}} className='bg-primary/20 hover:bg-green-800 hover:text-white text-primary  text-lg cursor-pointer font-bold rounded-lg    w-[90%] ml-4'>
                Join Now
              </Button>
      ...
      <div className="max-w-6xl gap-5 border-b border-muted text-white justify-evenly h-96 flex-1 flex items-center absolute     mx-auto">
          ...
          <div className="text-white    gap-4 h-full w-1/3">
            <span className="  items-center w-full ">
              <span className='flex items-center gap-2'>
                ...
                <h2  style={{fontFamily:'Quicksand'}} className='text-white text-xl font-bold'>Seeds of Love</h2>
              </span>
              <p  style={{fontFamily:'Quicksand'}} className='text-sm mt-4  text-muted'>It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.</p>
               
            </span>

            <span  style={{fontFamily:'Quicksand'}} className='flex items-center gap-3 mt-5'>
              {socialMediaLinks.map((social, i) => (
                <Link  style={{fontFamily:'Quicksand'}} key={i} href={social.url} target="_blank" className='w-10 h-10 rounded-full border-2 border-white flex items-center justify-center text-white bg-primary hover:bg-green-800  hover:text-white'>
          ...
          <div className="text-white    gap-4 h-full w-1/3">
            <span className="  items-center w-full ">
              <h2  style={{fontFamily:'Quicksand'}} className='text-white text-xl font-bold'>Quick Links</h2>
              <ul className='text-sm mt-4 text-justify text-muted'>
                <li><Link  style={{fontFamily:'Quicksand'}} href="/about" className='hover:text-primary text-lg  mt-5 transition-colors'>About Us</Link></li>
                <li><Link  style={{fontFamily:'Quicksand'}} href="/blog" className='hover:text-primary text-lg  mt-5 transition-colors'>Blog</Link></li>
                <li><Link  style={{fontFamily:'Quicksand'}} href="/contact" className='hover:text-primary text-lg  mt-5 transition-colors'>Contact</Link></li>
              </ul>
            </span>
          </div>
          ...
          <div className="text-white    gap-4 h-full w-1/3">
            <span className="  items-center w-full font-bold text-accent">
              <h2  style={{fontFamily:'Quicksand'}} className='text-white font-bold'>Contact us</h2>
              <ul className='text-sm mt-4 text-justify text-muted'>
                <li className='flex mt-5 items-center gap-2'>
                  <span className='text-primary'><MapPin /></span>
                  <span  style={{fontFamily:'Quicksand'}}>123 Charity Street, City, Country</span>
                </li>
                <li className='flex mt-5 items-center gap-2'>
                  <span className='text-primary'><PhoneCall/> </span>
                  <span  style={{fontFamily:'Quicksand'}}>+1 (123) 456-7890</span>
                </li>
                <li className='flex mt-5 items-center gap-2'>
                  <span className='text-primary'><Mail /></span>
                  <span  style={{fontFamily:'Quicksand'}}>info@seedsoflove.org</span>
                </li>
              </ul>
            </span>
          </div>
        ...
        <span className='absolute border flex border-border rounded-full p-2 gap-2 bottom-40 right-100'>
          <Input  style={{fontFamily:'Quicksand'}} placeholder='Subscribe to our newsletter'
            className='bg-transparent rounded-full  border-0 p-5 text-sm text-white w-80' />
          <Button  style={{fontFamily:'Quicksand'}} className='bg-primary hover:bg-green-800 text-white px-8 text-lg cursor-pointer font-bold rounded-full p-5'>
            <Send size={18} />
          </Button>
        </span>

         <span className='text-sm text-muted w-1/4 mt-5 text-center   flex   justify-evenly absolute bottom-10 left-5'>
          <Link  style={{fontFamily:'Quicksand'}} href="/privacy-policy" className='cursor-pointer  hover:text-primary transition-colors'>Privacy Policy</Link>
          <Link  style={{fontFamily:'Quicksand'}} href="/terms-of-service" className='cursor-pointer hover:text-primary transition-colors'>Terms of Service</Link>
        </span>

        <span  style={{fontFamily:'Quicksand'}} className='text-sm text-muted mt-5 text-center w-full absolute bottom-10'>
